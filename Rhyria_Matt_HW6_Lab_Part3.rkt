...
#| Note: I tried it like this but it didnt work apparently the order matters 
(define-type Class
      ...
      (type-case Exp a
        ...
        [(newE class-name field-exprs)
         (local [(define c (if(equal? class-name 'Object) 
                              (classC 'Object empty empty)  ;; add 'Object part 3
...
(define-type ExpI
  ...
  (selectI [num-expr : ExpI]
            [obj-expr : ExpI])
  (instanceI [obj-expr : ExpI]  ;; part 3 add instanceI
             [class-name : Symbol]))
    ...
    (type-case ExpI a
      ...
      [(superI method-name arg-expr)
       (ssendE (thisE)
               ...
               method-name
               (recur arg-expr))]
       [(selectI num-expr obj-expr)   ;; part 2
       (selectE (recur num-expr) (recur obj-expr))]
      [(instanceI obj-expr class-name)  ;; part 3 add instanceI
...
(define (class-i->c-not-flat [c : ClassI]) : Class
  (type-case ClassI c
    [(classI super-name field-names methods) ;; add super-name if not added
     (classC
      super-name  ;; add super-name
      field-names
      (map (lambda (m)
             (values (fst m)
  ...
  (type-case Class (find classes-not-flat name)
    [(classC super-name field-names methods)   ;; add super name part 3
     (type-case Class (flatten-super name classes-not-flat i-classes)
       [(classC supper super-field-names super-methods)
        (classC
         super-name  ;; add super name part 3
...
(define (flatten-super [name : Symbol]
  ...
  (type-case ClassI (find i-classes name)
    [(classI super-name field-names i-methods) ;; add super-name
     (if (equal? super-name 'Object)  ;; add super name part 3
         (classC 'Object empty empty)  ;; add 'Object part 3
...
;; there is a test case here somewhere that u need to remove
  ...
  (define posn-i-class
    (values
     'Posn
     (classI 'Object  ;; part 3 add'Object not sure it was already add here
             (list 'x 'y)
             (list (values 'mdist
                           (plusI (getI (thisI) 'x)
                                  (getI (thisI) 'y)))
  ...
  (define posn-c-class-not-flat
    (values
     'Posn
    (classC 'Object (list 'x 'y)  ;; part 3 add'Object same thing here
            (list (values 'mdist
                          (plusE (getE (thisE) 'x)
                                 (getE (thisE) 'y)))
                  addDist-c-method))))
